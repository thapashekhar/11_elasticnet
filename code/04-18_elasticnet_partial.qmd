...
weather_recipe <-
  ...
  recipe(strength_gtex ~ ., data = weather_train) %>%
...
enet_spec  <-
  ...
  #linear_reg(penalty = 10, mixture = .4) %>%  for fixed value
  linear_reg(penalty = tune(), mixture = tune()) %>%  # for tunining 
  # Specify the engine
  set_engine("glmnet")
...
enet_grid <- crossing(penalty = seq(0, 
                      ...
                      mixture = seq(0, 
                                    ...
                                    by = 0.2))
library(dials)
enet_grid <- grid_regular(penalty(),mixture(), levels = list(penalty = 50, mixture =10))
...
```{r}
ggplot(data = enet_grid, aes(x = mixture, y = penalty))+
  geom_point()
```
...
```{r resampling_foldcv}
resampling_foldcv <- vfold_cv(weather_train, v = 5)
...
```{r enet_grid_result}
enet_grid_result <- tune_grid(enet_spec, preprocessor = weather_recipe, grid = enet_grid,
                              resamples = resampling_foldcv)

enet_grid_result
enet_grid_result$.metrics[[2]]
```
